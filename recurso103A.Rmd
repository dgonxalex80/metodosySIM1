...
Antes de resumir la información contenida en una data en tablas de frecuencia dependiendo el tipo de variable revisamos incialmente la estructura de la data, lu cual nos perite un diagnóstico incial para ello utilizaremos una de las bases contenidas en el paquete `paqueteMET` para lo cual debemos instalarlo de la siguiente manera :
...
En ocaciones es necesario seleccionar una parte de la base de datos para estos casos podemos utilizar la función 
...
Se pueden emplear carias cociones
...
La función `table()` permite la construcción de tablas para dos variables o tablas cruzadas para frecuencias absolutas o tambien para frecuencias relativas (si se multiplican por 100) 
...
La función `freq()` del paquete `summarytools` permite una construcción más elaborada de la tabla que incluye ademas de las frecuencias simples tambien las acumuladas
