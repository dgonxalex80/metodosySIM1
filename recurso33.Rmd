...
set.seed(1)
df <- data.frame(rbind(rep(10, 8), # limite superior de la escala
                       rep(0, 8), #  limite inferior de la escala
                       matrix(sample(0:10, 8),nrow = 1))) # genera una muestra de valores
colnames(df) <- paste("Var", 1:8)   # genera las etiquetas de las variables
...
library(fmsb)
# matriz de datos
x = c(2.3, 2.9, 1.5, 4.0, 3.8, 4.0, 3.7, 3.2, 
      3.1, 4.0, 2.0, 3.6, 3.2, 3.3, 1.2, 4.0, 
      3.8, 4.0, 3.9, 3.5, 1.5, 3.0, 3.4, 4.0)

puntos = matrix(x, nrow = 3, byrow = TRUE)

df2 <- data.frame(rbind(rep(4, 8), 
                        rep(1, 8),
                        puntos))
...
plot <- ggplot(data,aes(x = variable,y = value,
          ...
          labs(x = "", y = "",title = "  ",   # add titulo, subtitulo, nota
                             subtitle = " ", 
                             caption = " " ) +
