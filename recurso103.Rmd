...
Arreglos multidimensionales de valores. 
...
```{r, echo=FALSE}
h=hist(rnorm(100,45,5), col = "#F7C490", main = "Distribución del tiempo de carrera K10", las=1, xlab = "tiempo en min", ylab ="frecuencia") 
```
...
Estructura de datos de dos dimensiones - filas y columnas - base de datos. con la función `data.frame()` se puede generar un objeto.  
...
R contiene base de datos con estructura data.frame, por ejemplo : `data(iris)` carga la base iris que contiene 150 registros (columnas) y 5 variables (filas).
...
<pre>
x=seq(from=2,to=10, by=0.01)  # genera secuencia de números entre 2 y 10 con paso 0.01
...
#  <span style="color:#034a94">**Funciones útilies**</span>
...
each = 1)         # número de veces que se replica elementos individuales del objeto
...
along.with=NULL)    # longitud del vector
</pre>
